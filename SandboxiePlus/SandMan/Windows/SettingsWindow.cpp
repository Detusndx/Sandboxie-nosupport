...
// QByteArray g_Certificate; // NOSUPPORT
...
void CSettingsWindow::UpdateCert()
{
	...
	{
		// ui.txtCertificate->setPlainText(g_Certificate); // NOSUPPORT
		ui.txtCertificate->setPlainText("Here goes the certificate"); // NOSUPPORT
...
void CSettingsWindow::ApplyCert()
{
	...
	if (false) { // (g_Certificate != Certificate) { // NOSUPPORT
...
void CSettingsWindow::LoadCertificate(QString CertPath)
{
	...
	if (CertFile.open(QFile::ReadOnly)) {
		//g_Certificate = CertFile.readAll(); // NOSUPPORT
		CertFile.close();
	}
}
